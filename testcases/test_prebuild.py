...
def test_prebuild_with_vscode_repo(playwright : Playwright):
    ...
    with page.expect_popup() as new_page_info:
        page.get_by_role("form").filter(has_text="Create a codespace on main").get_by_role("button").click()
        page.wait_for_timeout(30000)
    new_page = new_page_info.value
    new_page.wait_for_selector(".notification-list-item-message", state="detached")
